...
for k = 1:Sim_samples
    ...
    % 2. Update the prediction model along the horizon
    for j = 1:n
        % Page j maps the state at stage j-1 to the state at stage j
        if j == 1
            h1_j = x_prev(1);
            h2_j = x_prev(2);
        else
    ...
    % Update only A; B is constant and no disturbance model is used
    mpc = update_mpc_dynamics(mpc,A_lpv,[],[]);

    % 3. Solve using the current state, previous input, output reference, and
