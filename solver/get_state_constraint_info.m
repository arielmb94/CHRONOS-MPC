function [mpc,s,s_all,s_ter,u,du,y,err,h,feas] = ...
...
if feas && ~isempty(mpc.h_cnstr)
    ...
    mpc.h_cnstr = fi_box_fun(mpc.h_cnstr,h,mpc.Nh,mpc.nh,0);
    feas = fi_box_is_feasible(mpc.h_cnstr);
end

% Terminal Constraint
if feas && mpc.ter_ingredients && mpc.ter_constraint
    mpc.fi_ter_x0 = get_terConst_val(x_ref,s_ter,mpc.P,0);
    if mpc.fi_ter_x0>=0 
        feas = 0;
    end
end
