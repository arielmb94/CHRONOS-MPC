...
if cnstr.min_limit
    ...
    candidate_v = max([mpc.slack_epsilon*ones(n,1),...
        max_fi_across_horizon + mpc.slack_epsilon*1.1,...
        cnstr.min_v],[], 2);
    cnstr.min_v = candidate_v;
    % Map to global v vector
    v(cnstr.min_v_global_index) = cnstr.min_v;
...
if cnstr.max_limit
    ...
    candidate_v = max([mpc.slack_epsilon*ones(n,1),...
        max_fi_across_horizon + mpc.slack_epsilon*1.1,...
        cnstr.max_v],[], 2);
    cnstr.max_v = candidate_v;
    % Map to global v vector
    v(cnstr.max_v_global_index) = cnstr.max_v;
